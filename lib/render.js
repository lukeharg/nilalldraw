...
function storyHtml(story, { showSection = false } = {}) {
  ...
  <p class="meta"><span class="src">${esc(story.source)}</span><span class="dot">·</span><span class="time">${fmtTime.format(new Date(story.published))}</span>${
    showSection && section
...
function page({ base, title, active, edition, body, footerExtra = "", canonicalPath = "" }) {
  const navTabs = [
...
(() => {
  ...
  const paint = () => {
    ...
    if (root.dataset.theme) for (const m of metas) m.content = dark ? "#15190F" : "#EFEDE2";
  };
...
export function renderSite({ stories, feeds, now, dist, root }) {
  ...
  const editionNo = Math.floor(Number(fmtHour.format(now)) / 6) + 1;
  const edition = `${dayLabel(now.toISOString(), now)} · Edition ${editionNo} of 4`;
